...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <xsl:template match="poemTitle">
    ...
    <xsl:template match="line"><lb num="{@n}"/><xsl:apply-templates/></xsl:template>
    
    
    
    <xsl:template match="poem"><poem xml:id="p-{count(preceding-sibling::poem)+1}"><xsl:apply-templates/></poem>
        </xsl:template>
